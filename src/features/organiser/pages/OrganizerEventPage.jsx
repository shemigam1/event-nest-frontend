 ...
 * Lives at /organiser/events/:id, reached from My events → Manage.
 *
 * Layout:
 *   • Header — back link, role/status badges, title + when/where,
 *              big Sold and Checked-in numbers
 *   • Tab bar — Live dashboard / Attendees / Staff / Settings
 *   • Tab body — live tiles + per-tier bars + bookings chart + activity
 *                feed (live tab), or the corresponding management
...
function PageHeader({ event, totalSold, totalCapacity, checkedIn, checkInRate }) {
    ...
    return (
        <div>
            ...
            <div className="mp-num" style={{
                fontSize: 30, fontWeight: 700, lineHeight: 1.1, marginTop: 2,
...
function DashboardTab({
    ...
    totalSold, totalCapacity, checkedIn, checkInRate, isPublished,
}) {
    // Seed "now" once on mount so React's purity rules don't complain about
    // calling Date.now() inside useMemo. The page already refetches when
    // bookings change, so a stable mount-time anchor is the right tradeoff.
    const [nowMs] = useState(() => Date.now());
    // Per-hour deltas, derived from the bookings list. The backend doesn't
    // expose hourly slices, so we bucket client-side.
    ...
    // Pending update / rejection banners — kept from the previous design,
    // surfaced here as cross-cutting information rather than in a tab.
        ...
        <>
            ...
            <div className="mp-dashboard-grid" style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20, minWidth: 0 }}>
                    ...
                    {/* Empty / draft state — no analytics yet */}
...
function Tile({ label, value, sub, icon, delta }) {
    return (
        <div style={{
            ...
            <div className="mp-num" style={{
                fontSize: 26, fontWeight: 700, color: 'var(--text-1)', lineHeight: 1,
            }}>
                {value}
            </div>
            {(sub || delta) && (
                ...
                {tiers.map(tier => {
                    ...
                    return (
                        <div key={tier.id}>
                            ...
                            <div style={{
                                height: 8, background: 'var(--surface-subtle)',
...
function BookingsChart({ points }) {
    // Restrict to the most recent 24 entries — backend provides daily
    // buckets, which is the best granularity we have without an hourly
    // endpoint. The "last 24 hours" framing is approximate.
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                <span style={{ fontSize: 11, color: 'var(--text-3)' }}>Daily</span>
            </div>
            <div style={{
                    ...
                    return (
                        <div
                            ...
                            style={{
                                flex: 1,
                                height: `${h}%`,
                                background: last ? 'var(--mp-blue)' : 'rgba(37,99,235,0.25)',
                                borderRadius: 3,
                                minHeight: 4,
                            }}
                        />
...
/**
 * Compact money formatter — mirrors the screenshot (e.g. ₦11,900K).
 * Falls back to a plain ₦X for small values.
 */
